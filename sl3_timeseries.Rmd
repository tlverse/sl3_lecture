...
ts_task <- sl3_Task$new(data = bsds, covariates = covars,
...
```{r}
# make learner object
lrnr_arima <- Lrnr_arima$new(n.ahead = 5)
...
```{r}
lrnr_tsdyn_linear <- Lrnr_tsDyn$new(learner = "linear", m = 1, n.ahead = 5)
lrnr_tsdyn_setar <- Lrnr_tsDyn$new(learner = "setar", m = 1, model = "TAR",
                                   n.ahead = 5)
lrnr_tsdyn_lstar <- Lrnr_tsDyn$new(learner = "lstar", m = 1, n.ahead = 5)
```
...
```{r}
ts_uni_stack <- Stack$new(lrnr_arima, lrnr_tsdyn_linear, lrnr_tsdyn_setar,
                      lrnr_tsdyn_lstar)
ts_uni_stack_fit <- ts_uni_stack$train(ts_task)
ts_uni_stack_preds <- ts_uni_stack_fit$predict()
head(ts_uni_stack_preds)
```
...
```{r}
cv_ts_uni_stack <- Lrnr_cv$new(ts_uni_stack)
cv_ts_fit <- cv_ts_uni_stack$train(ts_task)
...
cv_ts_task <- cv_ts_fit$chain()
ts_ml_fit <- metalearner$train(cv_ts_task)
```
...
```{r}
sl_ts_pipeline <- make_learner(Pipeline, ts_uni_stack_fit, ts_ml_fit)
...
```{r}
lrnr_sl <- Lrnr_sl$new(learners = ts_uni_stack, metalearner = metalearner)
ts_sl_fit <- lrnr_sl$train(ts_task)
